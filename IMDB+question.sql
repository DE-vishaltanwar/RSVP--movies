...
SELECT COUNT(*) AS single_genre_moviecount
...
FROM   director_mapping d INNER JOIN names n ON d.name_id=n.id INNER JOIN genre g ON g.movie_id=d.movie_id INNER JOIN ratings r ON g.movie_id=r.movie_id
WHERE genre IN(SELECT genre FROM top_genre ) AND r.avg_rating>8
GROUP BY name
ORDER BY movie_count DESC
LIMIT 3;
...
SELECT n.name as actor_name,COUNT(m.movie_id) movie_count
FROM   names n INNER JOIN role_mapping m ON n.id=m.name_id  INNER JOIN ratings r ON r.movie_id=m.movie_id 
WHERE  r.median_rating>=8 AND m.category='Actor'
GROUP BY actor_name
ORDER BY movie_count DESC
LIMIT 2; 
...
SELECT production_company, SUM(total_votes) AS vote_count, RANK() OVER(ORDER BY SUM(total_votes) DESC)
FROM   movie m INNER JOIN ratings r ON m.id=r.movie_id
GROUP BY production_company
LIMIT 3;
...
WITH actor_summary AS (
	...
	FROM 	movie AS m INNER JOIN ratings AS r ON m.id = r.movie_id INNER JOIN role_mapping rm ON m.id = rm.movie_id 
...
WITH actress_summary AS (
	...
	FROM 	movie AS m INNER JOIN ratings r ON m.id=r.movie_id INNER JOIN role_mapping rm ON m.id = rm.movie_id 
...
WITH thriller_movies AS (
	...
	FROM	movie m INNER JOIN ratings r ON r.movie_id = M.id INNER JOIN genre g ON r.movie_id=g.movie_id
...
WITH top_genres AS(
	...
	FROM movie m INNER JOIN genre g ON g.movie_id = m.id INNER JOIN ratings r ON r.movie_id = m.id
	WHERE avg_rating > 8
...
WITH actress_summary AS(
	...
	FROM movie m INNER JOIN ratings r ON m.id=r.movie_id INNER JOIN role_mapping rm ON m.id = rm.movie_id 
			INNER JOIN names n ON rm.name_id = n.id INNER JOIN GENRE g ON g.movie_id = m.id
...
WITH next_date_published_summary AS(
	...
	FROM 	director_mapping d INNER JOIN names n ON n.id = d.name_id INNER JOIN movie m ON m.id = d.movie_id
